...
int main() {
	...
	state_map.insert(make_pair("MP", 57));
	state_map.insert(make_pair("UP", 93));
	state_map.insert(make_pair("AndraPradesh", 37));
	state_map.insert(make_pair("ArunachalPradesh", 17));
	...
	state_map.insert(make_pair("Bihar", 35));
	...
	while(m!=0){
		...
		if(state_map.find(state) != state_map.end()){
			cout<<"The population of the state "
                <<state
                <<" is : "
                <<state_map[state]
                <<" million. "<<endl;
